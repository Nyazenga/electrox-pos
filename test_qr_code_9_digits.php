<?php
/**
 * Test QR code generation with 9-digit receiptGlobalNo
 */
...
echo "=== TESTING QR CODE WITH 9-DIGIT receiptGlobalNo ===\n\n";
...
// Generate QR code with 9-digit receiptGlobalNo
try {
    ...
    // Check the length of receiptGlobalNo in the URL
    $urlParts = parse_url($qrResult['qrCode']);
    $path = $urlParts['path'] ?? '';
    
    // Extract receiptGlobalNo (should be 9 digits after deviceID (10) + date (8))
    // Format: /deviceID(10)receiptDate(8)receiptGlobalNo(9)receiptQrData(16)
    $qrData = str_replace($urlParts['scheme'] . '://' . $urlParts['host'] . '/', '', $qrResult['qrCode']);
    
    if (strlen($qrData) >= 27) { // 10 (device) + 8 (date) + 9 (global) = 27 minimum
        $devicePart = substr($qrData, 0, 10);
        $datePart = substr($qrData, 10, 8);
        $globalNoPart = substr($qrData, 18, 9); // Changed from 10 to 9
        $qrDataPart = substr($qrData, 27, 16);
        
        echo "QR Code Breakdown:\n";
        echo "  Device ID (10 digits): $devicePart\n";
        echo "  Receipt Date (8 digits): $datePart\n";
        echo "  Receipt Global No (9 digits): $globalNoPart\n";
        echo "  QR Data (16 digits): $qrDataPart\n\n";
        
        if (strlen($globalNoPart) === 9) {
            echo "✓ SUCCESS: receiptGlobalNo is correctly formatted as 9 digits!\n";
        } else {
            echo "✗ ERROR: receiptGlobalNo should be 9 digits, but found " . strlen($globalNoPart) . " digits\n";
        }
    }
