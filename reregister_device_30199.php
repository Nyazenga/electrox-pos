...
try {
    // Store certificate directly using our database connection
    if ($isLocalhost) {
        ...
        if ($cert) {
            ...
            if (isset($certData['validTo_time_t'])) {
                $validTill = date('Y-m-d H:i:s', $certData['validTo_time_t']);
            }
        }
        
        // Update fiscal_devices table directly
        $updateSql = "UPDATE fiscal_devices SET 
            certificate_pem = :cert, 
            private_key_pem = :key, 
            certificate_valid_till = :valid_till,
            is_registered = 1,
            updated_at = NOW()
