# PowerShell script to test ZIMRA OpenDay API endpoint
# Usage: .\test_open_day_api.ps1 [device_id] [fiscal_day_no]
# Example: .\test_open_day_api.ps1 30200
# Example: .\test_open_day_api.ps1 30200 101
...
$phpScript = Join-Path $scriptDir "test_open_day_api.php"
...
Write-Host "ZIMRA OpenDay API Test" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
Write-Host ""
Write-Host "API Endpoint: POST /Device/v1/$DeviceId/OpenDay" -ForegroundColor Cyan
...
try {
    ...
    foreach ($line in $output) {
        ...
        elseif ($lineStr -match "SUCCESS" -or $lineStr -match "Operation ID" -or $lineStr -match "Fiscal Day No" -or $lineStr -match "Test completed successfully") {
            $isSuccess = $true
        }
