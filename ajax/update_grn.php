...
try {
    ...
    foreach ($items as $item) {
        ...
        if ($requiresSpecificList && !empty($specificListEntries)) {
            // Delete existing entries for this GRN item (if updating)
            $db->executeQuery("DELETE FROM product_specific_list WHERE grn_item_id = :grn_item_id", [':grn_item_id' => $itemId]);
