...
$auth->requireLogin();
$auth->requireAnyPermission('reports.general', 'reports.view', 'reports.highest_movers_sales_contribution');

$pageTitle = 'Highest Movers by Sales Contribution';
...
$totalNetSales = $totalSales ? floatval($totalSales['total_net_sales']) : 0;

// Get highest movers by sales contribution
$highestMovers = $db->getRows("SELECT 
    ...
    CASE 
        WHEN :total_sales > 0 
        THEN (COALESCE(SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0)), 0) / :total_sales) * 100
        ELSE 0
...
LIMIT $limit", array_merge($params, [':total_sales' => $totalNetSales]));

if ($highestMovers === false) {
    $highestMovers = [];
}

// Recalculate contribution percentage with actual total
foreach ($highestMovers as &$product) {
    if ($totalNetSales > 0) {
        $product['sales_contribution_percent'] = ($product['net_sales'] / $totalNetSales) * 100;
    } else {
        $product['sales_contribution_percent'] = 0;
    }
}
unset($product);
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Highest Movers by Sales Contribution</h2>';
    ...
    $html .= '<tr><td>Total Net Sales</td><td style="text-align: right;">' . formatCurrency($totalNetSales) . '</td></tr>';
    $html .= '<tr><td>Top ' . $limit . ' Products Contribution</td><td style="text-align: right;">' . number_format(array_sum(array_column($highestMovers, 'sales_contribution_percent')), 2) . '%</td></tr>';
    $html .= '</table>';
    
    $html .= '<h3 style="margin-top: 30px; margin-bottom: 10px;">Top ' . $limit . ' Products by Sales Contribution</h3>';
    $html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 9px;">';
    $html .= '<tr style="background-color: #f0f0f0;"><th>Rank</th><th>Product Code</th><th>Product Name</th><th>Category</th><th>Branch</th><th style="text-align: right;">Qty Sold</th><th style="text-align: right;">Net Sales</th><th style="text-align: right;">Contribution %</th></tr>';
    $rank = 1;
    foreach ($highestMovers as $product) {
        $html .= '<tr>';
        $html .= '<td>' . $rank++ . '</td>';
        ...
        $html .= '<td style="text-align: right;">' . number_format($product['sales_contribution_percent'], 2) . '%</td>';
        $html .= '</tr>';
    }
    $html .= '</table>';
    
    ReportHelper::generatePDF('Highest Movers by Sales Contribution', $html, 'Highest_Movers_Sales_Contribution_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-pie-chart"></i> Highest Movers by Sales Contribution</h2>
    <div>
    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-body text-center">
                ...
                <h3 class="mb-0"><?= formatCurrency($totalNetSales) ?></h3>
            </div>
        </div>
    </div>
    <div class="col-md-6">
        <div class="card">
            <div class="card-body text-center">
                <h6 class="text-muted mb-2">Top <?= $limit ?> Contribution</h6>
                <h3 class="mb-0 text-primary"><?= number_format(array_sum(array_column($highestMovers, 'sales_contribution_percent')), 2) ?>%</h3>
            </div>
        </div>
    </div>
</div>
            ...
            <table class="table table-striped table-hover" id="highestMoversTable">
                ...
                <tbody>
                    <?php if (empty($highestMovers)): ?>
                        <tr>
                            <td colspan="8" class="text-center text-muted py-4">No products found for the selected period</td>
                        </tr>
                    <?php else: ?>
                        <?php $rank = 1; foreach ($highestMovers as $product): ?>
                            <tr>
                                ...
                                <td class="text-end"><?= number_format($product['total_quantity_sold'], 2) ?></td>
                                <td class="text-end fw-bold"><?= formatCurrency($product['net_sales']) ?></td>
                                <td class="text-end"><span class="badge bg-info"><?= number_format($product['sales_contribution_percent'], 2) ?>%</span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
