...
$auth->requireLogin();
$auth->requireAnyPermission('reports.general', 'reports.view', 'reports.highest_movers_gross_profit');

$pageTitle = 'Highest Movers by Gross Profit';
...
$summary = $db->getRow("SELECT 
    ...
    COALESCE(SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0) - (si.quantity * COALESCE(p.cost_price, 0))), 0) as total_gross_profit,
...
// Get highest movers by gross profit
$highestMovers = $db->getRows("SELECT 
    ...
    COALESCE(SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0) - (si.quantity * COALESCE(p.cost_price, 0))), 0) as gross_profit,
    CASE 
        WHEN SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0)) > 0 
        THEN (SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0) - (si.quantity * COALESCE(p.cost_price, 0))) / SUM(si.total_price - COALESCE(s.discount_amount * (si.total_price / NULLIF(s.subtotal, 0)), 0))) * 100
        ELSE 0
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Highest Movers by Gross Profit</h2>';
    ...
    $html .= '<h3 style="margin-top: 30px; margin-bottom: 10px;">Top ' . $limit . ' Products by Gross Profit</h3>';
    $html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 9px;">';
    $html .= '<tr style="background-color: #f0f0f0;"><th>Rank</th><th>Product Code</th><th>Product Name</th><th>Category</th><th>Branch</th><th style="text-align: right;">Qty Sold</th><th style="text-align: right;">Net Sales</th><th style="text-align: right;">Cost</th><th style="text-align: right;">Gross Profit</th><th style="text-align: right;">Margin %</th></tr>';
    $rank = 1;
    ...
    ReportHelper::generatePDF('Highest Movers by Gross Profit', $html, 'Highest_Movers_Gross_Profit_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-graph-up-arrow"></i> Highest Movers by Gross Profit</h2>
    <div>
    ...
    <div class="col-md-4">
        <div class="card">
            <div class="card-body text-center">
                ...
                <h3 class="mb-0 text-success"><?= formatCurrency($summary['total_gross_profit']) ?></h3>
            </div>
        </div>
    </div>
    <div class="col-md-4">
            ...
            <table class="table table-striped table-hover" id="highestMoversTable">
                ...
                <tbody>
                    <?php if (empty($highestMovers)): ?>
                        <tr>
                            <td colspan="10" class="text-center text-muted py-4">No products found for the selected period</td>
                        </tr>
                    <?php else: ?>
                        <?php $rank = 1; foreach ($highestMovers as $product): ?>
                            <tr>
                                ...
                                <td class="text-end"><span class="badge bg-info"><?= number_format($product['profit_margin'], 2) ?>%</span></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
