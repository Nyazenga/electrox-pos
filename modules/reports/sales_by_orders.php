...
$auth->requireLogin();
$auth->requireAnyPermission('reports.advanced_sales', 'reports.view', 'reports.sales', 'reports.sales_by_order');

$pageTitle = 'Sales by Orders Report';
...
// Get sales by orders - select only needed columns
$salesByOrders = $db->getRows("SELECT s.id, s.receipt_number, s.sale_date, s.total_amount, s.payment_status,
                               i.invoice_number,
...
if ($salesByOrders === false) {
    $salesByOrders = [];
}

// PDF Export
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Sales by Orders Report</h2>';
    ...
    $html .= '<h3 style="margin-top: 30px; margin-bottom: 10px;">Sales Orders</h3>';
    $html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 9px;">';
    $html .= '<tr style="background-color: #f0f0f0;"><th>Receipt #</th><th>Invoice #</th><th>Date</th><th>Customer</th><th>Cashier</th><th>Branch</th><th style="text-align: right;">Amount</th><th>Status</th></tr>';
    foreach ($salesByOrders as $order) {
        $html .= '<tr>';
        $html .= '<td>' . escapeHtml($order['receipt_number']) . '</td>';
        $html .= '<td>' . escapeHtml($order['invoice_number'] ?? 'N/A') . '</td>';
        $html .= '<td>' . date('M d, Y', strtotime($order['sale_date'])) . '</td>';
        $html .= '<td>' . escapeHtml(($order['customer_first'] ?? 'Walk-in') . ' ' . ($order['customer_last'] ?? '')) . '</td>';
        $html .= '<td>' . escapeHtml(($order['cashier_first'] ?? '') . ' ' . ($order['cashier_last'] ?? '')) . '</td>';
        $html .= '<td>' . escapeHtml($order['branch_name'] ?? 'N/A') . '</td>';
        $html .= '<td style="text-align: right;">' . formatCurrency($order['total_amount']) . '</td>';
        $html .= '<td>' . escapeHtml($order['payment_status']) . '</td>';
        $html .= '</tr>';
    }
    $html .= '</table>';
    
    ReportHelper::generatePDF('Sales by Orders Report', $html, 'Sales_by_Orders_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-cart-check"></i> Sales by Orders Report</h2>
    <div>
    ...
    <div class="card-body">
        <form method="GET" class="row g-3">
            ...
            <div class="col-12">
                ...
                <a href="sales_by_orders.php" class="btn btn-secondary"><i class="bi bi-arrow-clockwise"></i> Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card">
    ...
    <div class="card-body">
        <div class="table-responsive">
            <table class="table table-striped table-hover data-table" id="salesByOrdersTable">
                <thead>
                    <tr>
                        <th>Receipt #</th>
                        <th>Invoice #</th>
                ...
                <tbody>
                    <?php if (empty($salesByOrders)): ?>
                        <tr>
                            <td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($salesByOrders as $order): ?>
                            <tr>
                                <td><?= escapeHtml($order['receipt_number']) ?></td>
                                <td><?= escapeHtml($order['invoice_number'] ?? 'N/A') ?></td>
                                <td><?= date('M d, Y', strtotime($order['sale_date'])) ?></td>
                                <td><?= escapeHtml(($order['customer_first'] ?? 'Walk-in') . ' ' . ($order['customer_last'] ?? '')) ?></td>
                                <td><?= escapeHtml(($order['cashier_first'] ?? '') . ' ' . ($order['cashier_last'] ?? '')) ?></td>
                                <td><?= escapeHtml($order['branch_name'] ?? 'N/A') ?></td>
                                <td class="text-end"><?= formatCurrency($order['total_amount']) ?></td>
                                <td><span class="badge bg-<?= $order['payment_status'] === 'paid' ? 'success' : 'warning' ?>"><?= escapeHtml($order['payment_status']) ?></span></td>
                                <td><a href="<?= BASE_URL ?>modules/pos/receipt.php?id=<?= $order['id'] ?>" class="btn btn-sm btn-outline-primary" target="_blank"><i class="bi bi-eye"></i></a></td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                </tbody>
