...
$auth->requireLogin();
$auth->requireAnyPermission('reports.general', 'reports.view', 'reports.view_stock_transfers');

$pageTitle = 'View Stock Transfers';
...
if ($transfersTableExists) {
    ...
    if (!empty($search)) {
        $whereConditions[] = "(st.transfer_number LIKE :search OR p.product_code LIKE :search OR p.product_name LIKE :search)";
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Stock Transfers</h2>';
    $html .= '<p style="text-align: center; color: #666;">Generated: ' . date('M d, Y H:i') . '</p>';
    
    if (!$transfersTableExists || empty($transfers)) {
        $html .= '<p style="text-align: center; color: #666;">No stock transfers found or stock transfers feature not available.</p>';
    } else {
    ...
    ReportHelper::generatePDF('Stock Transfers', $html, 'Stock_Transfers_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-arrow-left-right"></i> View Stock Transfers</h2>
    <div>
...
<?php else: ?>
        ...
        <div class="card-body">
            <form method="GET" class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="search" value="<?= escapeHtml($search) ?>" class="form-control" placeholder="Search transfers...">
                </div>
                ...
                <table class="table table-striped table-hover" id="transfersTable">
                    ...
                    <tbody>
                        <?php if (empty($transfers)): ?>
                            <tr>
                                <td colspan="9" class="text-center text-muted py-4">No stock transfers found</td>
                            </tr>
                        <?php else: ?>
    ...
    function viewTransfer(transferId) {
        // Open transfer view in new window or modal
        window.open('<?= BASE_URL ?>modules/inventory/transfer_view.php?id=' + transferId, '_blank');
    }
    </script>
<?php endif; ?>
