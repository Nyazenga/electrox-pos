...
$auth->requireLogin();
$auth->requireAnyPermission('reports.general', 'reports.view', 'reports.sales_period_all_users');

$pageTitle = 'Sales for Period All Users';
...
// Get sales by user with daily breakdown
...
WHERE $whereClause
GROUP BY u.id, u.first_name, u.last_name, u.email, DATE(s.sale_date)
ORDER BY DATE(s.sale_date) DESC, net_sales DESC", $params);
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Sales for Period All Users</h2>';
    ...
    $html .= '<h3 style="margin-top: 30px; margin-bottom: 10px;">Sales by User (Daily Breakdown)</h3>';
    $html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 8px;">';
    $html .= '<tr style="background-color: #f0f0f0;"><th>Date</th><th>User</th><th>Email</th><th>Sales Count</th><th style="text-align: right;">Gross Sales</th><th style="text-align: right;">Discounts</th><th style="text-align: right;">Net Sales</th></tr>';
    foreach ($salesByUser as $user) {
        $html .= '<tr>';
        $html .= '<td>' . date('M d, Y', strtotime($user['sale_date'])) . '</td>';
    ...
    ReportHelper::generatePDF('Sales for Period All Users', $html, 'Sales_Period_All_Users_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-people-fill"></i> Sales for Period All Users</h2>
    <div>
            ...
            <table class="table table-striped table-hover" id="salesByUserTable">
                ...
                <tbody>
                    <?php if (empty($salesByUser)): ?>
                        <tr>
                            <td colspan="7" class="text-center text-muted py-4">No sales found for the selected period</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($salesByUser as $user): ?>
                            <tr>
                                <td><?= date('M d, Y', strtotime($user['sale_date'])) ?></td>
