...
$auth->requireLogin();
$auth->requireAnyPermission('reports.general', 'reports.view', 'reports.sales_by_category_user');

$pageTitle = 'Sales by Category for User';
...
// Get sales by category and user
...
WHERE $whereClause
GROUP BY u.id, u.first_name, u.last_name, u.email, pc.id, pc.name
ORDER BY u.first_name, u.last_name, net_sales DESC", $params);
...
if (isset($_GET['export']) && $_GET['export'] === 'pdf') {
    $html = '<h2 style="text-align: center; margin-bottom: 20px;">Sales by Category for User</h2>';
    ...
    $html .= '<h3 style="margin-top: 30px; margin-bottom: 10px;">Sales by Category and User</h3>';
    $html .= '<table border="1" cellpadding="5" cellspacing="0" style="width: 100%; font-size: 8px;">';
    $html .= '<tr style="background-color: #f0f0f0;"><th>User</th><th>Email</th><th>Category</th><th>Sales</th><th style="text-align: right;">Qty</th><th style="text-align: right;">Net Sales</th></tr>';
    foreach ($salesByCategory as $item) {
        $html .= '<tr>';
        $html .= '<td>' . escapeHtml($item['user_name'] ?? 'N/A') . '</td>';
        $html .= '<td>' . escapeHtml($item['email'] ?? 'N/A') . '</td>';
    ...
    ReportHelper::generatePDF('Sales by Category for User', $html, 'Sales_By_Category_User_' . date('Ymd') . '.pdf');
    exit;
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <h2><i class="bi bi-tags"></i> Sales by Category for User</h2>
    <div>
            ...
            <table class="table table-striped table-hover" id="salesByCategoryTable">
                ...
                <tbody>
                    <?php if (empty($salesByCategory)): ?>
                        <tr>
                            <td colspan="6" class="text-center text-muted py-4">No sales found for the selected period</td>
                        </tr>
                    <?php else: ?>
                        <?php foreach ($salesByCategory as $item): ?>
                            <tr>
                                <td><span class="fw-bold"><?= escapeHtml($item['user_name'] ?? 'N/A') ?></span></td>
