 ...
 * NOTE: Temporarily re-enabled for testing - ZIMRA support requested concatenated string for 5% tax
 * 
 * @param array $taxes Array of tax objects
 * @return array Filtered taxes (currently returns all taxes including 5%)
 */
function filterOut5PercentTax($taxes) {
    ...
    // TEMPORARILY RE-ENABLED: ZIMRA support requested to test 5% tax fiscalization
    // Return all taxes including 5% for testing
    return $taxes;
...
function mapApplicableTaxes($applicableTaxesRaw, $hasTaxID = false) {
    ...
    // TEMPORARILY RE-ENABLED: ZIMRA support requested to test 5% tax fiscalization
    // Filter function now returns all taxes including 5% for testing
    ...
    foreach ($applicableTaxesRaw as $tax) {
        ...
        // For 5% Non-VAT Withholding Tax: use empty taxCode (per Python library and documentation)
...
function fiscalizeSale($saleId, $branchId, $db = null) {
    ...
    try {
        ...
        foreach ($saleItems as $item) {
            ...
            // NOTE: taxID 514 (5% Non-VAT Withholding Tax) may have empty taxCode - this is valid
            ...
            // CRITICAL LOGGING: Verify taxCode assignment for receiptLine (especially 5% tax)
            if ($taxId == 514 && $taxPercent == 5) {
                writeFiscalLog("FISCALIZE SALE: *** RECEIPTLINE TAXCODE VERIFICATION - 5% NON-VAT WITHHOLDING TAX ***");
                writeFiscalLog("FISCALIZE SALE: taxID=514, taxPercent=5, taxCode='" . $taxCode . "' (SHOULD BE EMPTY STRING '')");
                error_log("FISCALIZE SALE: *** RECEIPTLINE - 5% NON-VAT WITHHOLDING TAX - taxID=514, taxCode='" . $taxCode . "' (SHOULD BE EMPTY '') ***");
            }
        ...
        foreach ($taxGroups as $group) {
            ...
            // NOTE: taxID 514 (5% Non-VAT Withholding Tax) may have empty taxCode - this is valid
            if (empty($validTax['taxID'])) {
                throw new Exception("Invalid tax configuration: Missing required field (taxID) in ZIMRA applicable tax (taxID={$validTax['taxID']}, taxPercent={$validTax['taxPercent']}, taxCode={$validTax['taxCode']}). Please sync configuration from ZIMRA.");
            }
            ...
            if ($validTax['taxID'] == 514 && $taxPercentForAPI == 5) {
                ...
                writeFiscalLog("FISCALIZE SALE: receiptTaxEntry taxCode='" . $receiptTaxEntry['taxCode'] . "' (for signature generation)");
