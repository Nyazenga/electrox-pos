 ...
 * This will find and update all electrox_* databases
 */
...
try {
    ...
    // Get all databases
    $allDatabases = $pdo->query("SHOW DATABASES")->fetchAll(PDO::FETCH_COLUMN);
    
    // Filter to only tenant databases (electrox_*)
    $tenantDatabases = array_filter($allDatabases, function($db) {
        return strpos($db, 'electrox_') === 0 && $db !== 'electrox_base';
    });
    
    echo "Found " . count($tenantDatabases) . " tenant database(s) to process\n\n";
    ...
    foreach ($tenantDatabases as $dbName) {
        ...
        try {
            ...
            $tableExists = $tenantPdo->query("SHOW TABLES LIKE 'products'");
            if ($tableExists->rowCount() == 0) {
                echo "  ⚠ Products table doesn't exist, skipping...\n\n";
                continue;
            }
            
            // Check if requires_specific_list column exists
            ...
            $checkSource = $tenantPdo->query("SHOW COLUMNS FROM products LIKE 'source'");
            if ($checkSource->rowCount() > 0) {
                echo "  ✓ Column 'source' already exists\n";
            } else {
                // Add column
                $tenantPdo->exec("ALTER TABLE `products` ADD COLUMN `source` varchar(50) DEFAULT 'manual' COMMENT 'Source of product: manual, import, bulk, etc.'");
